...
def setup_model_fc(model):
    model.fc = nn.Sequential(
        nn.Linear(in_features=1024, out_features=6, bias=True),  # 추가된 중간 레이어
    ).to(device)
...
class Parameters:
    def __init__(self, batch_size, learning_rate):
        self.description = "Googlenet for Image Classification"
        # 에포크 수
        self.epochs = 50
        # 배치 크기
        ...
        # 훈련된 모델 경로
        self.model_name = f"Googlenet_{batch_size}_{learning_rate}"
...
for lr in [1e-3, 1e-4]:
    for batch_size in [8, 16, 32, 64, 128]:
        ...
        for param in model.inception4c.parameters():
            param.requires_grad = True
